...
static int rtk_i2c_init_recovery_info(struct rtk_i2c_dev *priv)
{
    ...
    {
        dev_err(dev, "can't get pin default state, bus recovery not support\n");
		return -1;
    }

	if (IS_ERR(priv->pinctrl_gpio)) 
    {
		dev_err(dev, "can't get pin gpio_mode state, bus recovery not support\n");
		return -1;
	}
